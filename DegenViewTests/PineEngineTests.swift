...
final class PineEngineTests: XCTestCase {
    private func bars(_ closes:[Double])->[KlineData] { closes.enumerated().map { i,c in .init(openTime:Date(timeIntervalSince1970:Double(i)*60),openPrice:c,highPrice:c+1,lowPrice:c-1,closePrice:c,volume:Double(i+1)) } }
    ...
    func testRequiredScriptsCompileAndExecute() throws {
        ...
        for source in scripts { let program=PineCompiler.compile(source:source);XCTAssertTrue(program.isValid,"\(program.diagnostics)");let result=try PineRuntimeSession(program:program).evaluate(bars:bars([1,2,3,4,5,6]));XCTAssertFalse(result.output.plots.isEmpty) }
    }
    ...
    func testRealtimeRollbackAndVarip() throws {
        ...
        let session=PineRuntimeSession(program:PineCompiler.compile(source:source));let first=bars([10])[0]
        try session.execute(.init(candle:first,phase:.historical))
        var open=bars([11])[0];open = .init(openTime:Date(timeIntervalSince1970:60),openPrice:11,highPrice:11,lowPrice:11,closePrice:11,volume:1)
        try session.execute(.init(candle:open,phase:.realtimeTick(isNew:true)))
        open.closePrice=12;try session.execute(.init(candle:open,phase:.realtimeTick(isNew:false)))
        let values=session.output().plots.sorted{$0.id<$1.id}.map{$0.values.last!}
        XCTAssertEqual(values[0],2);XCTAssertEqual(values[1],3)
    }

    func testV6Diagnostics() {
        XCTAssertEqual(PineCompiler.compile(source:"//@version=5\nindicator(\"x\")").diagnostics.first?.code,"PINE0002")
