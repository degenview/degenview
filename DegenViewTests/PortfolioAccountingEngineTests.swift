...
final class PortfolioAccountingEngineTests: XCTestCase {
    ...
    private func tx(
        _ portfolio: UUID, _ asset: PortfolioAsset, _ type: PortfolioTransactionType,
        _ quantity: Decimal, _ price: Decimal? = nil, fee: Decimal = 0,
        day: TimeInterval = 0, externalID: String? = nil
    ) -> PortfolioTransaction {
        .init(
            portfolioID: portfolio, asset: asset, type: type, quantity: quantity, price: price,
    ...
    func testConcreteWeightedAverageAndValuation() throws {
        ...
        let holdings = try PortfolioAccountingEngine.holdings(
            transactions: transactions, portfolioIDs: [p1],
        ...
        XCTAssertEqual(value.costBasis, 120_000)
        XCTAssertEqual(value.averageCost, 40_000)
        XCTAssertEqual(value.currentValue, 150_000)
        XCTAssertEqual(value.unrealizedPnL, 30_000)
        XCTAssertEqual(value.pnlPercent, Decimal(string: "0.25"))
    }

    func testPortfolioQuoteDictionaryCodableRoundTrip() throws {
        let timestamp = Date(timeIntervalSince1970: 1_735_700_123)
        let values = [
            btc.key: PortfolioQuote(
                price: Decimal(string: "98765.4321")!,
                previousDayPrice: Decimal(string: "95432.10"), timestamp: timestamp)
        ]
        let decoded = try JSONDecoder().decode(
            [String: PortfolioQuote].self,
    ...
    func testInitialQuoteLoadingDecisions() {
        ...
        XCTAssertFalse(
            PortfolioStore.needsInitialQuoteLoad(
                snapshot: usdSnapshot,
                quotes: [btc.key: staleQuote]))

        let partialSnapshot = PortfolioLedgerSnapshot(
            portfolios: [usd],
        ...
        unsupportedAsset = PortfolioAsset(
            key: unsupportedAsset.key, symbol: unsupportedAsset.symbol,
    ...
    func testPartialSellRealizedAndRemainingPnL() throws {
        let transactions = [
            tx(p1, btc, .buy, 1, 30_000), tx(p1, btc, .buy, 2, 45_000, day: 1), tx(p1, btc, .sell, 1, 55_000, day: 2),
        ]
        let h = try XCTUnwrap(
            PortfolioAccountingEngine.holdings(
                transactions: transactions, portfolioIDs: [p1],
                quotes: [btc.key: .init(price: 50_000, timestamp: Date())]
            ).first)
        XCTAssertEqual(h.quantity, 2)
        XCTAssertEqual(h.averageCost, 40_000)
        XCTAssertEqual(h.realizedPnL, 15_000)
        XCTAssertEqual(h.currentValue, 100_000)
        XCTAssertEqual(h.unrealizedPnL, 20_000)
    }
    ...
    func testFeesIncreaseBuyBasisAndReduceSellProceeds() throws {
        ...
        XCTAssertEqual(h.quantity, 1)
        XCTAssertEqual(h.averageCost, 105)
        XCTAssertEqual(h.realizedPnL, 40)
    }
    ...
    func testTransferOutIsNotRealizedSale() throws {
        ...
        let h = try XCTUnwrap(PortfolioAccountingEngine.holdings(transactions: transactions, portfolioIDs: [p1]).first)
        XCTAssertEqual(h.quantity, 1)
        XCTAssertEqual(h.costBasis, 102)
        XCTAssertEqual(h.realizedPnL, 0)
    }
    ...
    func testHistoricalHoldingsAreTransactionAware() throws {
        ...
        let january = try PortfolioAccountingEngine.holdings(
            transactions: transactions, portfolioIDs: [p1], through: jan1.addingTimeInterval(14 * 86_400))
        let february = try PortfolioAccountingEngine.holdings(
            transactions: transactions, portfolioIDs: [p1], through: jan1.addingTimeInterval(45 * 86_400))
        XCTAssertEqual(january.map(\.asset.key), [btc.key])
        XCTAssertEqual(Set(february.map(\.asset.key)), [btc.key, eth.key])
    }
    ...
    func testMultiplePortfoliosStayIndependentAndAggregate() throws {
        ...
        XCTAssertEqual(
            try PortfolioAccountingEngine.holdings(transactions: values, portfolioIDs: [p1]).first?.quantity, 1)
        let all = try PortfolioAccountingEngine.holdings(transactions: values, portfolioIDs: [p1, p2])
        XCTAssertEqual(all.first { $0.asset.key == btc.key }?.quantity, 3)
        XCTAssertEqual(all.first { $0.asset.key == eth.key }?.quantity, 5)
    }

    func testAllocationUsesCurrentValues() throws {
        let values = [tx(p1, btc, .buy, 1, 10), tx(p1, eth, .buy, 2, 10)]
        let holdings = try PortfolioAccountingEngine.holdings(
            transactions: values, portfolioIDs: [p1],
            quotes: [btc.key: .init(price: 80, timestamp: Date()), eth.key: .init(price: 10, timestamp: Date())])
    ...
    func testEditInvalidatesOnlyAffectedSnapshotSuffix() async throws {
        ...
        let before = PortfolioSnapshot(
            portfolioID: id, timestamp: jan1.addingTimeInterval(-86_400), value: 0, netContributions: 0, realizedPnL: 0,
            unrealizedPnL: 0, isComplete: true)
        let after = PortfolioSnapshot(
            portfolioID: id, timestamp: jan1.addingTimeInterval(86_400), value: 30_000, netContributions: 30_000,
            realizedPnL: 0, unrealizedPnL: 0, isComplete: true)
        try await ledger.storeSnapshots([before, after], for: id, from: .distantPast)
        original.quantity = 2
        try await ledger.update(original)
        let snapshot = await ledger.snapshot()
        XCTAssertTrue(snapshot.historicalSnapshots.contains(before))
        XCTAssertFalse(snapshot.historicalSnapshots.contains(after))
        XCTAssertEqual(snapshot.invalidatedAfter[id], jan1)
        XCTAssertEqual(
            try PortfolioAccountingEngine.holdings(transactions: snapshot.transactions, portfolioIDs: [id]).first?
                .quantity, 2)
    }
    ...
    func testAggregateHistoryHasStableIdentityAndTotals() throws {
        ...
        let second = PortfolioSnapshot(
            portfolioID: p2, timestamp: first.timestamp,
    ...
    private func snapshotPoint(portfolio: UUID, day: TimeInterval) -> PortfolioSnapshot {
        PortfolioSnapshot(
            portfolioID: portfolio, timestamp: jan1.addingTimeInterval(day * 86_400),
    ...
    func testPortfolioCreationDuplicationDeletionAndPersistenceCodable() async throws {
        let ledger = PortfolioLedger(now: { self.jan1 })
        let a = try await ledger.createPortfolio(name: "A", currency: .USD)
        try await ledger.add(tx(a, btc, .buy, 1, 10))
        let b = try await ledger.duplicatePortfolio(a)
        var state = await ledger.snapshot()
        XCTAssertEqual(state.portfolios.count, 2)
        XCTAssertEqual(state.transactions.first { $0.portfolioID == b }?.quantity, 1)
        try await ledger.deletePortfolio(a)
        state = await ledger.snapshot()
        XCTAssertEqual(state.portfolios.map(\.id), [b])
        XCTAssertFalse(state.transactions.contains { $0.portfolioID == a })
    ...
    func testCSVPreviewIsAtomicAndDeduplicatesExternalIDs() async throws {
        ...
        } catch { XCTAssertEqual(error as? PortfolioError, .duplicateExternalTransaction) }
    ...
    func testPrivacyNeverReturnsSensitiveVisibleOrAccessibilityText() {
        ...
        XCTAssertEqual(
            PortfolioPrivacy.accessibility(
                "Portfolio balance, $128,430", enabled: true, hiddenDescription: "Portfolio balance hidden"),
    ...
    func testCoinMarketCapImportParsesTimezoneQuotedThousandsAndMissingFees() throws {
        ...
        XCTAssertEqual(preview.rows[0].price, Decimal(string: "69608.61"))
        XCTAssertEqual(preview.rows[0].amount, Decimal(string: "0.001328"))
        XCTAssertEqual(preview.rows[1].amount, Decimal(string: "1012712.00"))
        XCTAssertEqual(preview.rows[1].fee, 0)
        XCTAssertEqual(ISO8601DateFormatter().string(from: preview.rows[0].timestamp), "2026-08-20T07:25:00Z")

        let transactions = preview.transactions(
            portfolioID: p1,
            mappings: [
                "BTC": btc, "FLOKI": .init(key: "CoinGecko:floki", symbol: "FLOKI", name: "FLOKI", source: .coingecko),
            ])
        XCTAssertTrue(transactions.isValid)
        XCTAssertEqual(transactions.transactions.count, 2)
    ...
    func testCoinMarketCapImportSkipsUnmappedTickersAndRejectsUnconvertedForeignFee() {
        ...
        XCTAssertTrue(unmapped.isValid)
        XCTAssertTrue(unmapped.transactions.isEmpty)
        ...
        XCTAssertFalse(mapped.isValid)
        XCTAssertTrue(mapped.errors.contains { $0.contains("historical") })
        let rowID = try! XCTUnwrap(preview.rows.first?.id)
        let converted = preview.transactions(
            portfolioID: p1, mappings: ["FLOKI": asset], feeFXRates: [rowID: Decimal(string: "1.08")!])
        XCTAssertTrue(converted.isValid)
        XCTAssertEqual(converted.transactions.first?.fee, Decimal(string: "3.24"))
        ...
        XCTAssertTrue(skipped.transactions.isEmpty)
    }
    ...
    func testAutoMapperPrefersRequestedBinanceQuotePair() {
        ...
        XCTAssertEqual(
            PortfolioAssetAutoMapper.bestPair(in: results, token: "FLOKI", preferredQuotes: ["USDT", "USD"])?
                .fullSymbol, "FLOKIUSDT")
        XCTAssertEqual(
            PortfolioAssetAutoMapper.bestPair(in: results, token: "FLOKI", preferredQuotes: ["EUR", "USDT"])?
                .fullSymbol, "FLOKIEUR")
    }
    ...
    func testAssetRemapIsScopedAtomicAndRecalculatesLedger() async throws {
        ...
        let holding = try XCTUnwrap(
            PortfolioAccountingEngine.holdings(transactions: state.transactions, portfolioIDs: [p1]).first)
        XCTAssertEqual(holding.quantity, 1)
