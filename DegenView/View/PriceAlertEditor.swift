...
struct PriceAlertEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Picker("Condition", selection: $direction) {
                Text("Crosses Above").tag(0)
                Text("Crosses Below").tag(1)
                Text("Rises By %").tag(2)
                Text("Falls By %").tag(3)
            }
            if direction < 2 {
                ...
                if reference == nil {
                    Text("A fresh current price is required for percentage alerts.").font(.caption).foregroundStyle(
                        .orange)
                }
            }
            Picker("Currency", selection: $currency) {
                ForEach(PortfolioCurrency.alertCurrencies) { Text($0.rawValue).tag($0) }
            }
            Picker("Frequency", selection: $frequency) {
                ForEach(AlertFrequency.allCases) { Text($0.rawValue).tag($0) }
            }.pickerStyle(.segmented)
            TextField("Note (optional)", text: $note).textFieldStyle(.roundedBorder)
            if identical {
                Label("An identical alert already exists.", systemImage: "exclamationmark.triangle").font(.caption)
                    .foregroundStyle(.orange)
            }
            HStack {
                Spacer()
                Button("Cancel") { dismiss() }
                Button("Save") { Task { await save() } }.buttonStyle(.borderedProminent).disabled(condition == nil)
            }
        }
    ...
    private func populate() async {
        ...
        case .crossesAbove(let v):
            direction = 0
            target = v.description
        case .crossesBelow(let v):
            direction = 1
            target = v.description
        case .risesBy(let p, let r, _):
            direction = 2
            percent = p.description
            reference = r
        case .fallsBy(let p, let r, _):
            direction = 3
            percent = p.description
            reference = r
