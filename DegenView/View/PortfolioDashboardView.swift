...
struct PortfolioDashboardView: View {
    ...
    enum Tab: String, CaseIterable { case overview = "Overview", holdings = "Holdings", transactions = "Transactions", statistics = "Statistics" }
    ...
    var body: some View {
        ...
        .task { await store.refresh(); await store.refreshQuotes(); await store.rebuildHistory(); openInitialAssetIfNeeded() }
        ...
        .fileImporter(isPresented: $showImporter, allowedContentTypes: [.commaSeparatedText, .plainText]) { result in
            do {
                let url = try result.get(); guard url.startAccessingSecurityScopedResource() else { return }
                defer { url.stopAccessingSecurityScopedResource() }
                importPreview = PortfolioCSVService.preview(try String(contentsOf: url, encoding: .utf8), portfolios: store.snapshot.portfolios)
            } catch { store.lastError = error.localizedDescription }
        }
        .fileImporter(isPresented: $showCoinMarketCapImporter, allowedContentTypes: [.commaSeparatedText, .plainText]) { result in
            do {
                let url = try result.get(); guard url.startAccessingSecurityScopedResource() else { return }
                defer { url.stopAccessingSecurityScopedResource() }
                coinMarketCapPreview = PortfolioCSVService.previewCoinMarketCap(try String(contentsOf: url, encoding: .utf8))
            } catch { store.lastError = error.localizedDescription }
        }
        .fileExporter(isPresented: $showExporter, document: exportDocument, contentType: .commaSeparatedText,
                      defaultFilename: "degenview-portfolio-transactions.csv") { _ in }
        .confirmationDialog("Delete transaction?", isPresented: Binding(get: { deleteTransaction != nil }, set: { if !$0 { deleteTransaction = nil } })) {
            Button("Delete", role: .destructive) { if let id = deleteTransaction?.id { store.deleteTransaction(id) }; deleteTransaction = nil }
            Button("Cancel", role: .cancel) { deleteTransaction = nil }
        } message: { Text("Holdings, cost basis, P&L, allocation, and later history will be recalculated.") }
        .alert("Portfolio", isPresented: Binding(get: { store.lastError != nil }, set: { if !$0 { store.lastError = nil } })) {
    ...
    private var header: some View {
        HStack(spacing: 12) {
            Menu {
                ...
                Divider()
                ForEach(store.activePortfolios) { portfolio in Button(portfolio.name) { store.select(.portfolio(portfolio.id)) } }
                Divider(); Button("Create Portfolio…") { showCreate = true }; Button("Manage Portfolios…") { showManage = true }
            } label: { Label(store.selectedPortfolio?.name ?? "All Portfolios", systemImage: "chevron.down") }
            .accessibilityLabel("Portfolio selector, \(store.selectedPortfolio?.name ?? "All Portfolios")")
            Spacer()
            Button { store.privacyMode.toggle() } label: { Image(systemName: store.privacyMode ? "eye.slash" : "eye") }
            ...
            Menu {
                ...
                Button("Import from CoinMarketCap…") {
                    guard store.selectedPortfolio != nil else { store.lastError = "Select the destination portfolio before importing CoinMarketCap transactions."; return }
                    showCoinMarketCapImporter = true
                }
                Button("Export Transactions…") {
                    exportDocument = .init(text: PortfolioCSVService.exportTransactions(store.transactions, portfolios: store.snapshot.portfolios)); showExporter = true
                }
                Button("Export Current Holdings…") {
                    exportDocument = .init(text: PortfolioCSVService.exportHoldings(store.holdings, portfolioName: store.selectedPortfolio?.name ?? "All Portfolios")); showExporter = true
                }
                Button("Export Portfolio History…") {
                    exportDocument = .init(text: PortfolioCSVService.exportHistory(history)); showExporter = true
                }
    ...
    private var transactionsList: some View {
        VStack {
            HStack { TextField("Search asset, notes, or source", text: $transactionSearch); Picker("Type", selection: $transactionFilter) { Text("All").tag(nil as PortfolioTransactionType?); ForEach(PortfolioTransactionType.allCases) { Text($0.rawValue).tag(Optional($0)) } }.frame(width: 180) }.padding(.horizontal)
            List(filteredTransactions) { tx in
                HStack { Text(tx.asset.symbol).bold().frame(width: 100, alignment: .leading); Text(tx.type.rawValue).frame(width: 110, alignment: .leading); Text(store.privacyMode ? "••••" : tx.quantity.description).frame(width: 110); Text(tx.price.map { store.privacyMode ? "••••" : money($0) } ?? "—").frame(width: 110); Text(tx.timestamp.formatted(date: .abbreviated, time: .shortened)).frame(width: 170); Text(tx.notes).frame(maxWidth: .infinity, alignment: .leading); Text(tx.source.rawValue).foregroundStyle(.secondary); Menu { Button("Edit") { editor = .edit(tx) }; Button("Duplicate") { editor = .duplicate(tx) }; Button("Delete", role: .destructive) { deleteTransaction = tx } } label: { Image(systemName: "ellipsis.circle") } }
            }
        }
    }
    ...
    private var statistics: AnyView {
        ...
        return AnyView(ScrollView { LazyVGrid(columns: [.init(.flexible()), .init(.flexible()), .init(.flexible())], spacing: 16) {
            metric("Total P&L", store.totalPnL); metric("Realized P&L", store.totalRealizedPnL); metric("Unrealized P&L", store.totalUnrealizedPnL)
            textMetric("Best Performer", best.map { "\($0.asset.symbol) · \(percent($0.pnlPercent ?? 0))" } ?? "—")
            textMetric("Worst Performer", worst.map { "\($0.asset.symbol) · \(percent($0.pnlPercent ?? 0))" } ?? "—")
            textMetric("Number of Assets", "\(store.holdings.count)")
            textMetric("Portfolio High", history.map(\.value).max().map(privateMoney) ?? "—"); textMetric("Portfolio Low", history.map(\.value).min().map(privateMoney) ?? "—")
        }.padding() })
    }

    private var noPortfolios: some View { VStack(spacing: 14) { ContentUnavailableView("No Portfolios", systemImage: "briefcase", description: Text("Create a portfolio to start tracking investments.")); Button("Create Portfolio") { showCreate = true }.buttonStyle(.borderedProminent) }.frame(maxWidth: .infinity, maxHeight: .infinity) }
    private var emptyPortfolio: some View { VStack(spacing: 14) { ContentUnavailableView("Your portfolio is empty", systemImage: "chart.pie", description: Text("Track investments by adding your first transaction.")); HStack { Button("Add Transaction") { showAssetSearch = true }.buttonStyle(.borderedProminent); Button("Import Transactions") { showImporter = true } } }.frame(maxWidth: .infinity, maxHeight: .infinity) }
    private var destinationPortfolioID: UUID { store.selectedPortfolio?.id ?? store.activePortfolios.first!.id }
    private var sortedHoldings: [PortfolioHolding] { store.holdings.sorted { lhs, rhs in
    ...
    private var history: [PortfolioSnapshot] {
        ...
        return Dictionary(grouping: values, by: { calendar.startOfDay(for: $0.timestamp) }).map { date, points in
            PortfolioSnapshot(portfolioID: UUID(), timestamp: date, value: points.reduce(0) { $0 + $1.value },
                netContributions: points.reduce(0) { $0 + $1.netContributions }, realizedPnL: points.reduce(0) { $0 + $1.realizedPnL },
                unrealizedPnL: points.reduce(0) { $0 + $1.unrealizedPnL }, isComplete: points.allSatisfy(\.isComplete))
        }.sorted { $0.timestamp < $1.timestamp }
    }
    private var filteredTransactions: [PortfolioTransaction] { store.transactions.filter { tx in (transactionFilter == nil || tx.type == transactionFilter) && (transactionSearch.isEmpty || [tx.asset.name, tx.asset.symbol, tx.notes, tx.source.rawValue].contains { $0.localizedCaseInsensitiveContains(transactionSearch) }) }.sorted { $0.timestamp > $1.timestamp } }
    ...
    private func beginTransaction(for asset: PortfolioAsset) {
        guard !store.activePortfolios.isEmpty else { store.lastError = "Create a portfolio before adding a transaction."; return }
        editor = .new(asset: asset, portfolioID: destinationPortfolioID)
    }
    private func privateMoney(_ value: Decimal) -> String { PortfolioPrivacy.sensitive(money(value), enabled: store.privacyMode) }
    private func money(_ value: Decimal) -> String { let code = store.selectedPortfolio?.baseCurrency.rawValue ?? "USD"; return value.formatted(.currency(code: code).precision(.fractionLength(2))) }
    private func percent(_ value: Decimal) -> String { value.formatted(.percent.precision(.fractionLength(2))) }
    private func metric(_ title: String, _ value: Decimal) -> some View { textMetric(title, privateMoney(value)).foregroundStyle(value >= 0 ? Color.primary : Color.red) }
    private func textMetric(_ title: String, _ value: String) -> some View { VStack(alignment: .leading) { Text(title).font(.caption).foregroundStyle(.secondary); Text(value).font(.title3).bold() }.padding().frame(maxWidth: .infinity, alignment: .leading).background(.quaternary.opacity(0.4), in: RoundedRectangle(cornerRadius: 10)).eraseToAnyView() }
...
struct TransactionEditorContext: Identifiable {
    let id = UUID(); var transaction: PortfolioTransaction; var mode: Mode
    enum Mode { case new, edit, duplicate }
    static func new(asset: PortfolioAsset, portfolioID: UUID) -> Self { .init(transaction: .init(portfolioID: portfolioID, asset: asset, type: .buy, quantity: 0), mode: .new) }
    static func edit(_ value: PortfolioTransaction) -> Self { .init(transaction: value, mode: .edit) }
    static func duplicate(_ value: PortfolioTransaction) -> Self { var copy = value; copy = .init(portfolioID: copy.portfolioID, asset: copy.asset, type: copy.type, quantity: copy.quantity, price: copy.price, priceCurrency: copy.priceCurrency, fee: copy.fee, feeCurrency: copy.feeCurrency, timestamp: copy.timestamp, notes: copy.notes); return .init(transaction: copy, mode: .duplicate) }
}

private struct TransactionEditorSheet: View {
    @ObservedObject var store: PortfolioStore; let context: TransactionEditorContext
    @Environment(\.dismiss) private var dismiss
    @State private var type: PortfolioTransactionType; @State private var quantity: String; @State private var price: String; @State private var total: String; @State private var fee: String; @State private var date: Date; @State private var notes: String; @State private var editingTotal = false
    init(store: PortfolioStore, context: TransactionEditorContext) { self.store = store; self.context = context; let tx = context.transaction; _type = State(initialValue: tx.type); _quantity = State(initialValue: tx.quantity == 0 ? "" : tx.quantity.description); _price = State(initialValue: tx.price?.description ?? ""); _total = State(initialValue: tx.price.map { ($0 * tx.quantity).description } ?? ""); _fee = State(initialValue: tx.fee == 0 ? "" : tx.fee.description); _date = State(initialValue: tx.timestamp); _notes = State(initialValue: tx.notes) }
    var body: some View { VStack(alignment: .leading, spacing: 14) {
        Text("\(context.mode == .edit ? "Edit" : "Add") Transaction").font(.headline); Text("\(context.transaction.asset.name) · \(context.transaction.asset.source.rawValue)").foregroundStyle(.secondary)
        Picker("Type", selection: $type) { ForEach(PortfolioTransactionType.allCases) { Text($0.rawValue).tag($0) } }
        TextField("Quantity", text: $quantity).onChange(of: quantity) { recalculateTotal() }
        if [.buy, .sell, .transferIn, .reward, .stakingReward, .airdrop, .mining, .interest].contains(type) { TextField(type == .sell ? "Sale price" : "Price per asset (optional for transfers/rewards)", text: $price).onChange(of: price) { recalculateTotal() }; TextField("Total", text: $total).onChange(of: total) { _, _ in if editingTotal, let q = Decimal(string: quantity), q != 0, let t = Decimal(string: total) { price = (t / q).description } }.onTapGesture { editingTotal = true } }
        DatePicker("Date and time", selection: $date); TextField("Fee", text: $fee); TextField("Notes", text: $notes, axis: .vertical).lineLimit(2...4)
        HStack { Spacer(); Button("Cancel") { dismiss() }; Button("Save") { save() }.buttonStyle(.borderedProminent).disabled(Decimal(string: quantity).map { $0 <= 0 } ?? true) }
    }.padding(24).frame(width: 430) }
    private func recalculateTotal() { guard !editingTotal, let q = Decimal(string: quantity), let p = Decimal(string: price) else { return }; total = (q * p).description }
    private func save() { guard let q = Decimal(string: quantity) else { return }; var tx = context.transaction; tx.type = type; tx.quantity = q; tx.price = Decimal(string: price); tx.fee = Decimal(string: fee) ?? 0; tx.timestamp = date; tx.notes = notes; tx.updatedAt = Date(); Task { let ok = context.mode == .edit ? await store.update(tx) : await store.add(tx); if ok { dismiss(); await store.refreshQuotes(); await store.rebuildHistory() } } }
}

private struct PortfolioCreateSheet: View { @ObservedObject var store: PortfolioStore; @Environment(\.dismiss) var dismiss; @State var name = ""; @State var currency = PortfolioCurrency.USD
    var body: some View { VStack(spacing: 16) { Text("Create Portfolio").font(.headline); TextField("Name", text: $name); Picker("Base Currency", selection: $currency) { ForEach(PortfolioCurrency.allCases) { Text($0.rawValue).tag($0) } }; HStack { Button("Cancel") { dismiss() }; Button("Create") { store.create(name: name, currency: currency); dismiss() }.buttonStyle(.borderedProminent).disabled(name.trimmingCharacters(in: .whitespaces).isEmpty) } }.padding(24).frame(width: 360) }
}

private struct PortfolioManageSheet: View { @ObservedObject var store: PortfolioStore; @Environment(\.dismiss) var dismiss; @State var deleting: Portfolio?
    var body: some View { VStack { Text("Manage Portfolios").font(.headline); List { ForEach(store.snapshot.portfolios) { portfolio in HStack { TextField("Name", text: Binding(get: { portfolio.name }, set: { var copy = portfolio; copy.name = $0; store.update(copy) })); Spacer(); Button { store.duplicate(portfolio.id) } label: { Image(systemName: "plus.square.on.square") }; Button(role: .destructive) { deleting = portfolio } label: { Image(systemName: "trash") } } }.onMove { store.reorder(from: $0, to: $1) } }; Button("Done") { dismiss() } }.padding().frame(width: 520, height: 420).confirmationDialog("Delete \(deleting?.name ?? "portfolio")?", isPresented: Binding(get: { deleting != nil }, set: { if !$0 { deleting = nil } })) { Button("Delete Portfolio and Transactions", role: .destructive) { if let id = deleting?.id { store.delete(id) }; deleting = nil } } }
}

private enum PortfolioHistoryRange: String, CaseIterable, Identifiable {
    case oneDay = "1D", oneWeek = "1W", oneMonth = "1M", oneYear = "1Y", all = "ALL"
...
private struct PortfolioHistoryChart: View {
    ...
    private func draw(context: inout GraphicsContext, size: CGSize, points: [PortfolioSnapshot]) {
        ...
        for tick in 0...4 {
            ...
            context.draw(Text(shortMoney(value)).font(.caption2).foregroundStyle(.secondary),
                         at: CGPoint(x: plot.maxX + 6, y: y), anchor: .leading)
        }

        if points.count > 1 {
            for index in 1..<points.count {
                var segment = Path(); segment.move(to: position(index - 1)); segment.addLine(to: position(index))
            ...
            context.fill(Path(ellipseIn: CGRect(x: position(0).x - 3, y: position(0).y - 3, width: 6, height: 6)), with: .color(profit ? .green : .red))
        }

        let labelIndices = Array(Set([0, max(0, points.count / 2), max(0, points.count - 1)])).sorted()
        for index in labelIndices {
            context.draw(Text(dateLabel(points[index].timestamp)).font(.caption2).foregroundStyle(.secondary),
        ...
        context.stroke(vertical, with: .color(.secondary.opacity(0.65)), style: StrokeStyle(lineWidth: 1, dash: [4, 3]))
        context.stroke(horizontal, with: .color(.secondary.opacity(0.45)), style: StrokeStyle(lineWidth: 1, dash: [4, 3]))
        context.fill(Path(ellipseIn: CGRect(x: selectedPosition.x - 4, y: selectedPosition.y - 4, width: 8, height: 8)),
        ...
        context.draw(Text(tooltip).font(.caption).foregroundStyle(.primary),
...
private struct PortfolioAssetDetailView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack { VStack(alignment: .leading) { Text(holding.asset.name).font(.title).bold(); Text("\(holding.asset.symbol) · \(holding.asset.source.rawValue)").foregroundStyle(.secondary) }; Spacer(); Button("Done") { dismiss() } }
            HStack {
            ...
            HStack { detail("Unrealized P&L", hidden(holding.unrealizedPnL?.description ?? "—")); detail("Realized P&L", hidden(holding.realizedPnL.description)); detail("Total P&L", hidden(holding.totalPnL?.description ?? "—")); detail("Allocation", holding.allocation.formatted(.percent.precision(.fractionLength(2)))) }
            Text("Transactions").font(.headline)
            List(transactions) { tx in
                HStack { Text(tx.type.rawValue).frame(width: 110, alignment: .leading); Text(hidden(tx.quantity.description)).frame(width: 110); Text(tx.price.map { hidden($0.description) } ?? "—").frame(width: 100); Text(tx.timestamp.formatted(date: .abbreviated, time: .shortened)); Text(tx.notes).frame(maxWidth: .infinity, alignment: .leading); Menu { Button("Edit") { editor = .edit(tx) }; Button("Duplicate") { editor = .duplicate(tx) }; Button("Delete", role: .destructive) { deleting = tx } } label: { Image(systemName: "ellipsis.circle") } }
                    .accessibilityElement(children: .combine)
                    .accessibilityLabel(store.privacyMode ? "\(holding.asset.name) transaction, financial values hidden" : "\(tx.type.rawValue), \(tx.quantity) \(holding.asset.symbol)")
            }
        }.padding(24).frame(width: 850, height: 600)
        .sheet(item: $editor) { TransactionEditorSheet(store: store, context: $0) }
        .confirmationDialog("Delete transaction?", isPresented: Binding(get: { deleting != nil }, set: { if !$0 { deleting = nil } })) {
            Button("Delete", role: .destructive) { if let id = deleting?.id { store.deleteTransaction(id) }; deleting = nil }
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if !preview.warnings.isEmpty {
                DisclosureGroup("\(preview.warnings.count) warning\(preview.warnings.count == 1 ? "" : "s") — warnings do not block import") {
            ...
            if let importFailure {
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    Text("No transactions were imported. Correct the source data, or restart the CoinMarketCap import and skip the affected ticker or item.")
                        .font(.caption).foregroundStyle(.secondary)
                }
                .foregroundStyle(.red)
                .padding(10)
            ...
            Spacer()
            HStack {
                if isImporting { ProgressView(); Text("Validating and importing…").foregroundStyle(.secondary) }
                Spacer()
...
private struct CoinMarketCapImportSheet: View {
    ...
    private var mappedRows: Int { preview.rows.filter { mappings[$0.token] != nil && !skippedRowIDs.contains($0.id) }.count }
    ...
    private var foreignFeeRows: [CoinMarketCapCSVRow] {
        preview.rows.filter { row in
            mappings[row.token] != nil && !skippedRowIDs.contains(row.id) && row.fee != 0 && (row.feeCurrency ?? .USD) != .USD
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if !preview.errors.isEmpty {
                Label("\(preview.errors.count) CSV errors require attention", systemImage: "exclamationmark.triangle.fill").foregroundStyle(.red)
                List(preview.errors, id: \.self) { Text($0).font(.caption) }.frame(maxHeight: 100)
            }
            Text("Map CoinMarketCap tokens").font(.subheadline).bold()
            Text("Only transactions for mapped tickers will be imported. Leave a ticker skipped to exclude all of its transactions.")
                .font(.caption).foregroundStyle(.secondary)
            if !didAutoMap {
                HStack { ProgressView().controlSize(.small); Text("Finding \(portfolioCurrency.rawValue) markets — Binance first, then CoinGecko and DEXScreener…").font(.caption).foregroundStyle(.secondary) }
            }
                ...
                HStack {
                    ...
                    if resolvingSymbols.contains(symbol) {
                        ...
                        if autoMappedSymbols.contains(symbol) {
                            Text("Auto-mapped").font(.caption2).padding(.horizontal, 6).padding(.vertical, 2).background(.blue.opacity(0.12), in: Capsule())
                        }
                    ...
                    if mappings[symbol] != nil || skippedSymbols.contains(symbol) {
                        Button("Reset") { mappings[symbol] = nil; skippedSymbols.remove(symbol) }
                    }
                    Button(mappings[symbol] == nil ? "Map…" : "Change…") {
                        skippedSymbols.remove(symbol); autoMappedSymbols.remove(symbol); mappingSymbol = symbol
                    }
                    if mappings[symbol] == nil && !skippedSymbols.contains(symbol) {
            ...
            if !foreignFeeRows.isEmpty {
                GroupBox("Missing Historical FX") {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Enter the fee-currency value in USD at the transaction time. For example, if €1 equaled $1.08, enter 1.08. You can skip an individual transaction instead.")
                            .font(.caption).foregroundStyle(.secondary)
                        ScrollView {
                            VStack(spacing: 8) { ForEach(foreignFeeRows) { row in
                                HStack {
                                    Text("Line \(row.line) · \(row.token) · \(row.fee) \(row.feeCurrency?.rawValue ?? "")")
                                        .frame(maxWidth: .infinity, alignment: .leading)
                                    TextField("USD per \(row.feeCurrency?.rawValue ?? "unit")", text: Binding(
                                        get: { feeFXRateText[row.id] ?? "" }, set: { feeFXRateText[row.id] = $0 }
                                    )).frame(width: 145)
            ...
            if !skippedRowIDs.isEmpty {
                Button("Restore \(skippedRowIDs.count) individually skipped transaction\(skippedRowIDs.count == 1 ? "" : "s")") { skippedRowIDs.removeAll() }
                    .font(.caption)
            }
            HStack {
                Label("\(mappedRows) transactions will be imported", systemImage: "checkmark.circle.fill").foregroundStyle(.green)
                Spacer()
                Label("\(skippedRows) transactions will be skipped", systemImage: "minus.circle").foregroundStyle(.secondary)
            }.font(.callout)
            if !preview.warnings.isEmpty {
                DisclosureGroup("\(preview.warnings.count) warnings") {
                    ScrollView { VStack(alignment: .leading) { ForEach(preview.warnings, id: \.self) { Text($0).font(.caption) } } }.frame(maxHeight: 90)
                }
            }
            HStack {
                ...
                Button("Continue to Preview") {
                    ...
                    onReady(preview.transactions(portfolioID: portfolioID, mappings: mappings,
    ...
    private func applyExistingMappings() {
        for symbol in preview.symbols {
            let assets = Set(store.snapshot.transactions.filter { $0.asset.symbol.caseInsensitiveCompare(symbol) == .orderedSame }.map(\.asset))
...
struct PortfolioCSVDocument: FileDocument { static var readableContentTypes: [UTType] { [.commaSeparatedText] }; var text: String; init(text: String) { self.text = text }; init(configuration: ReadConfiguration) throws { text = configuration.file.regularFileContents.flatMap { String(data: $0, encoding: .utf8) } ?? "" }; func fileWrapper(configuration: WriteConfiguration) throws -> FileWrapper { .init(regularFileWithContents: Data(text.utf8)) } }
