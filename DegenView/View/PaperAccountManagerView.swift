...
struct PaperAccountManagerView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showCreate) {
            PaperAccountConfigurationSheet(title: "Create PAPER Account") { name, currency, balance, settings in
                Task { await store.createAccount(name: name, currency: currency, balance: balance, settings: settings) }
            }
        }
        .sheet(isPresented: $showReset) {
            PaperAccountConfigurationSheet(title: "Reset PAPER Account", destructive: true) {
                _, currency, balance, settings in
    ...
    private var header: some View {
        HStack {
            Text("PAPER").font(.caption.bold()).padding(.horizontal, 7).padding(.vertical, 3).background(
                .blue, in: Capsule()
            ).foregroundStyle(.white)
            Picker(
                "Paper account",
                selection: Binding(
                    get: { store.selectedAccount?.id }, set: { if let id = $0 { Task { await store.select(id) } } })
            ) {
            ...
            .buttonStyle(.plain)
            .accessibilityLabel(
                showTradingOnCharts ? "Hide trading controls on charts" : "Show trading controls on charts"
            )
            .help(showTradingOnCharts ? "Hide Trading on Charts" : "Show Trading on Charts")
            Menu {
                Button("Export trading data…") { exportCSV() }
                Divider()
                Button("Reset account…", role: .destructive) { showReset = true }
            } label: {
                Image(systemName: "ellipsis.circle")
            }.menuStyle(.borderlessButton)
            Button(action: onClose) { Image(systemName: "xmark") }.buttonStyle(.plain).accessibilityLabel(
                "Close Paper Trading panel")
        }
    }

    private var metrics: some View {
        HStack(spacing: 22) {
            metric("Balance", store.metrics?.balance)
            metric("Equity", store.metrics?.equity)
            metric("Realized P&L", store.metrics?.realizedPnL)
            metric("Unrealized P&L", store.metrics?.unrealizedPnL)
            metric("Account Margin", store.metrics?.positionMargin)
            metric("Orders Margin", store.metrics?.ordersMargin)
            ...
            Table(store.positions) {
                ...
                TableColumn("Actions") { position in
                    HStack {
                        ...
                        Button("Reverse") { Task { await store.reverse(position) } }
                    }
                }
            }
        case .orders:
            Table(store.workingOrders) {
                TableColumn("Symbol") { Text($0.instrument.symbol) }
                TableColumn("Side") { Text($0.side.rawValue.uppercased()) }
                TableColumn("Type") { Text($0.type.rawValue) }
                TableColumn("Quantity") { Text(quantity($0.originalQuantity, instrument: $0.instrument)) }
                TableColumn("Filled") { Text(quantity($0.filledQuantity, instrument: $0.instrument)) }
                TableColumn("Limit") { order in
                    Text(order.limitPrice.map { price($0, instrument: order.instrument) } ?? "—")
                }
                TableColumn("Stop") { order in
                    Text(order.stopPrice.map { price($0, instrument: order.instrument) } ?? "—")
                }
                ...
                TableColumn("") { order in
                    ...
                    }.accessibilityLabel("Cancel \(order.side.rawValue) \(order.type.rawValue) order")
                }
            }
        case .history:
            Table(store.orderHistory) {
                TableColumn("Time") { Text($0.timestamp.formatted(date: .omitted, time: .standard)) }
                TableColumn("Order ID") { Text($0.orderID.uuidString.prefix(8)) }
                TableColumn("Symbol") { Text($0.order.instrument.symbol) }
                TableColumn("Event") { Text($0.kind.rawValue) }
                TableColumn("Details") { Text($0.message) }
            }
        case .accountHistory:
            Table(store.closedTrades) {
                TableColumn("Symbol") { Text($0.instrument.symbol) }
                TableColumn("Side") { Text($0.side.rawValue) }
                TableColumn("Quantity") { Text(quantity($0.quantity, instrument: $0.instrument)) }
                TableColumn("Entry") { Text(price($0.entryPrice, instrument: $0.instrument)) }
                TableColumn("Exit") { Text(price($0.exitPrice, instrument: $0.instrument)) }
                TableColumn("Gross P&L") { Text(money($0.grossPnL)) }
                TableColumn("Commission") { Text(money($0.commission)) }
                TableColumn("Net P&L") { Text(money($0.netPnL)) }
            }
        case .journal:
            List(store.journal) { entry in
                HStack(alignment: .firstTextBaseline) {
                    Text(entry.timestamp.formatted(date: .abbreviated, time: .standard)).font(
    ...
    private func unrealized(_ position: PaperPosition) -> Decimal {
        store.snapshot.quotes[position.instrument.key].flatMap { quote in
            let mark = position.signedQuantity >= 0 ? (quote.bid ?? quote.last) : (quote.ask ?? quote.last)
            return mark.map {
                (position.signedQuantity >= 0 ? $0 - position.averageEntryPrice : position.averageEntryPrice - $0)
                    * position.quantity * position.instrument.pointValue
            }
        } ?? 0
    }
    private func money(_ value: Decimal) -> String {
        PaperTradingFormatter.money(value, currency: store.selectedAccount?.baseCurrency ?? .USD)
    }
    private func price(_ value: Decimal, instrument: PaperInstrument) -> String {
        PaperTradingFormatter.price(value, instrument: instrument)
    }
    private func quantity(_ value: Decimal, instrument: PaperInstrument) -> String {
...
struct PaperAccountConfigurationSheet: View {
    ...
    @State private var name = "Paper Trading"
    @State private var balance = "100000"
    @State private var currency: PaperCurrency = .USD
    @State private var leverage = "1"
    @State private var commissionType = 0
    @State private var commissionValue = "0"
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if destructive {
                Text(
                    "This permanently removes all positions, orders, fills, history, and journal entries for this paper account."
                ).font(.caption).foregroundStyle(.red)
            }
            TextField("Account name", text: $name).disabled(destructive)
            ...
            Picker("Commission", selection: $commissionType) {
                Text("None").tag(0)
                Text("Fixed per order").tag(1)
                Text("Percentage").tag(2)
                Text("Per contract").tag(3)
            }
            if commissionType != 0 { TextField("Commission value", text: $commissionValue) }
            HStack {
                Spacer()
                Button("Cancel", role: .cancel) { dismiss() }
                Button(destructive ? "Reset Account" : "Create Account", role: destructive ? .destructive : nil) {
                    submit()
    ...
    private func submit() {
        let amount = Decimal(string: balance) ?? 100_000
        let lev = max(1, Decimal(string: leverage) ?? 1)
        let fee = Decimal(string: commissionValue) ?? 0
        let commission: PaperCommissionConfiguration =
            switch commissionType {
            case 1: .fixedPerOrder(fee)
            case 2: .percentage(fee)
            case 3: .perContract(fee)
            default: .none
            }
        var settings = PaperAccountSettings()
        settings.commission = commission
        settings.leverage = .init(stocks: lev, crypto: lev, forex: lev, futures: lev, prediction: lev)
        completion(name, currency, amount, settings)
        dismiss()
    }
}
