...
final class ScriptEditorViewModel: ObservableObject {
    ...
    func load() {
        ...
        Task { do {
            ...
            name = script.name; type = script.type; source = script.source; savedSource = script.source
            status = script.compileRecord?.compilerVersion == ScriptStore.compilerVersion ? script.compileRecord?.status ?? .notCompiled : .notCompiled
            ...
            if let draft = try await ScriptStore.shared.draft(id: scriptID), draft.source != script.source {
                source = draft.source; isDirty = true
            }
    ...
    func compile() {
        let result = PineCompiler.compile(source: source); diagnostics = result.diagnostics
        status = result.diagnostics.contains { $0.severity == .error } ? .error :
    ...
    func save() {
        ...
        Task { do {
            ...
            savedSource = result.source; isDirty = false; status = result.compileRecord?.status ?? .notCompiled
