...
struct CoinMarketCapCSVPreview: Identifiable {
    ...
    func transactions(portfolioID: UUID, mappings: [String: PortfolioAsset],
                      feeFXRates: [String: Decimal] = [:], skippedRowIDs: Set<String> = []) -> PortfolioCSVPreview {
        ...
        if !skippedRows.isEmpty {
            ...
            conversionWarnings.append("Skipped \(skippedRows.count) transactions for unmapped tickers: \(symbols). Only mapped tickers will be imported.")
        }
        let values = rows.compactMap { row -> PortfolioTransaction? in
            ...
            if row.fee != 0 && feeCurrency != .USD {
                guard let rate = feeFXRates[row.id], rate > 0 else {
                    conversionErrors.append("Line \(row.line): supply the historical \(feeCurrency.rawValue) → USD rate or skip this transaction.")
                    return nil
                }
...
enum PortfolioCSVService {
    static let header = "portfolio,asset_id,symbol,name,source,type,quantity,price,currency,fee,fee_currency,date,notes,external_id"
    ...
    static func exportTransactions(_ transactions: [PortfolioTransaction], portfolios: [Portfolio]) -> String {
        ...
        return ([header] + transactions.sorted { $0.timestamp < $1.timestamp }.map { tx in
            [names[tx.portfolioID] ?? "", tx.asset.key, tx.asset.symbol, tx.asset.name,
             ...
             formatter.string(from: tx.timestamp), tx.notes, tx.externalTransactionID ?? ""].map(escape).joined(separator: ",")
        }).joined(separator: "\n")
    }

    static func exportHoldings(_ holdings: [PortfolioHolding], portfolioName: String, timestamp: Date = Date()) -> String {
        ...
        let rows = holdings.map { holding in
            [portfolioName, holding.asset.key, holding.asset.symbol, holding.quantity.description,
             ...
             holding.unrealizedPnL?.description ?? "", iso].map(escape).joined(separator: ",")
        }
        return (["portfolio,asset_id,symbol,quantity,average_cost,current_price,current_value,realized_pnl,unrealized_pnl,as_of"] + rows).joined(separator: "\n")
    }

    static func exportHistory(_ snapshots: [PortfolioSnapshot]) -> String {
        let iso = ISO8601DateFormatter()
        return (["portfolio_id,timestamp,value,net_contributions,realized_pnl,unrealized_pnl,is_complete"] + snapshots.sorted { $0.timestamp < $1.timestamp }.map {
            [$0.portfolioID.uuidString, iso.string(from: $0.timestamp), $0.value.description,
             ...
             $0.isComplete ? "true" : "false"].joined(separator: ",")
        }).joined(separator: "\n")
    }
    ...
    static func preview(_ csv: String, portfolios: [Portfolio]) -> PortfolioCSVPreview {
        ...
        guard let fields = rows.first else { return .init(transactions: [], errors: ["The CSV is empty."], warnings: []) }
        let indexes = Dictionary(fields.enumerated().map { ($1.lowercased(), $0) }, uniquingKeysWith: { existing, _ in existing })
        let required = ["portfolio", "asset_id", "symbol", "source", "type", "quantity", "currency", "date"]
        let missing = required.filter { indexes[$0] == nil }
        guard missing.isEmpty else { return .init(transactions: [], errors: ["Missing columns: \(missing.joined(separator: ", "))."], warnings: []) }
        var values: [PortfolioTransaction] = []; var errors: [String] = []; var warnings: [String] = []
        let iso = ISO8601DateFormatter()
        func value(_ row: [String], _ name: String) -> String { indexes[name].flatMap { row.indices.contains($0) ? row[$0] : nil } ?? "" }
        for (offset, row) in rows.dropFirst().enumerated() {
            let line = offset + 2
            guard let portfolio = portfolios.first(where: { $0.name.caseInsensitiveCompare(value(row, "portfolio")) == .orderedSame }) else { errors.append("Line \(line): unknown portfolio."); continue }
            guard let source = DataSourceType(rawValue: value(row, "source")),
                  ...
                  let date = iso.date(from: value(row, "date")) else { errors.append("Line \(line): invalid source, type, quantity, currency, or ISO-8601 date."); continue }
            let feeCurrency = PortfolioCurrency(rawValue: value(row, "fee_currency")) ?? currency
            let asset = PortfolioAsset(key: value(row, "asset_id"), symbol: value(row, "symbol"),
                name: value(row, "name").nilIfEmpty ?? value(row, "symbol"), source: source, quoteCurrency: currency)
            let tx = PortfolioTransaction(portfolioID: portfolio.id, asset: asset, type: type, quantity: quantity,
            ...
            values.append(tx)
            if tx.externalTransactionID == nil { warnings.append("Line \(line): no external_id; repeat imports cannot be deduplicated reliably.") }
        }
    ...
    static func previewCoinMarketCap(_ csv: String) -> CoinMarketCapCSVPreview {
        ...
        guard let dateIndex = column("date"), let tokenIndex = column("token"),
              ...
              let notesIndex = column("notes") else {
        ...
        var rows: [CoinMarketCapCSVRow] = [], errors: [String] = [], warnings: [String] = []
        func field(_ record: [String], _ index: Int) -> String { record.indices.contains(index) ? record[index].trimmingCharacters(in: .whitespacesAndNewlines) : "" }
        ...
        for (offset, record) in records.dropFirst().enumerated() {
            ...
            guard !token.isEmpty, let type,
                  ...
                  let total = decimal(field(record, totalIndex)) else {
            ...
            if fee != 0 && feeCurrency == nil { errors.append("Line \(line): fee currency is missing or unsupported."); continue }
            ...
            rows.append(.init(id: stableID, line: line, timestamp: date, token: token, type: type,
    ...
    private static func timeZone(from header: String) -> TimeZone? {
        guard let start = header.firstIndex(of: "+") ?? header.firstIndex(of: "-") else { return TimeZone(secondsFromGMT: 0) }
    ...
    private static func parseRow(_ row: String) -> [String] {
        var values: [String] = [], field = "", quoted = false, index = row.startIndex
        while index < row.endIndex {
            let char = row[index]
            if char == "\"" {
                let next = row.index(after: index)
                if quoted && next < row.endIndex && row[next] == "\"" { field.append("\""); index = next } else { quoted.toggle() }
            } else if char == "," && !quoted { values.append(field); field = "" } else { field.append(char) }
            index = row.index(after: index)
        }
        values.append(field); return values
    }
}
