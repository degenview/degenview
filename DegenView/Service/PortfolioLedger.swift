...
actor PortfolioLedger {
    ...
    init(snapshot: PortfolioLedgerSnapshot = .empty, now: @escaping @Sendable () -> Date = { Date() },
         persist: Persistence? = nil) {
        state = snapshot; self.now = now; self.persist = persist
    }
    ...
    func createPortfolio(name: String, currency: PortfolioCurrency) async throws -> UUID {
        let portfolio = Portfolio(name: name.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty ?? "Unnamed Portfolio",
                                  baseCurrency: currency, createdAt: now(), updatedAt: now())
        state.portfolios.append(portfolio); state.selectedPortfolioID = portfolio.id
        try await save(); return portfolio.id
    }
    ...
    func select(_ id: UUID?) async throws {
        ...
        state.selectedPortfolioID = id; try await save()
    }

    func updatePortfolio(_ portfolio: Portfolio) async throws {
        guard let index = state.portfolios.firstIndex(where: { $0.id == portfolio.id }) else { throw PortfolioError.portfolioNotFound }
        var value = portfolio; value.updatedAt = now(); state.portfolios[index] = value; try await save()
    }

    @discardableResult
    func duplicatePortfolio(_ id: UUID) async throws -> UUID {
        guard let original = state.portfolios.first(where: { $0.id == id }) else { throw PortfolioError.portfolioNotFound }
        let copy = Portfolio(name: "\(original.name) Copy", baseCurrency: original.baseCurrency, createdAt: now(), updatedAt: now())
        state.portfolios.append(copy)
        state.transactions += state.transactions.filter { $0.portfolioID == id }.map {
            PortfolioTransaction(portfolioID: copy.id, asset: $0.asset, type: $0.type, quantity: $0.quantity,
        ...
        state.selectedPortfolioID = copy.id; invalidate(copy.id, from: state.transactions.filter { $0.portfolioID == copy.id }.map(\.timestamp).min() ?? now())
        try await save(); return copy.id
    }
    ...
    func deletePortfolio(_ id: UUID) async throws {
        ...
        state.portfolios.removeAll { $0.id == id }; state.transactions.removeAll { $0.portfolioID == id }
        state.historicalSnapshots.removeAll { $0.portfolioID == id }; state.invalidatedAfter[id] = nil
    ...
    func add(_ transaction: PortfolioTransaction) async throws {
        guard state.portfolios.contains(where: { $0.id == transaction.portfolioID }) else { throw PortfolioError.portfolioNotFound }
        try PortfolioAccountingEngine.validate(transaction, in: state.transactions)
        state.transactions.append(transaction); touch(transaction.portfolioID); invalidate(transaction.portfolioID, from: transaction.timestamp)
        try await save()
    }

    func update(_ transaction: PortfolioTransaction) async throws {
        guard let index = state.transactions.firstIndex(where: { $0.id == transaction.id }) else { throw PortfolioError.transactionNotFound }
        ...
        state.transactions[index] = transaction; touch(transaction.portfolioID)
        invalidate(old.portfolioID, from: min(old.timestamp, transaction.timestamp))
        if old.portfolioID != transaction.portfolioID { invalidate(transaction.portfolioID, from: transaction.timestamp) }
        try await save()
    }

    func deleteTransaction(_ id: UUID) async throws {
        guard let transaction = state.transactions.first(where: { $0.id == id }) else { throw PortfolioError.transactionNotFound }
        ...
        state.transactions = remaining; touch(transaction.portfolioID); invalidate(transaction.portfolioID, from: transaction.timestamp)
        try await save()
    }
    ...
    func remapAsset(from sourceKey: String, to destination: PortfolioAsset,
        ...
        where portfolioIDs.contains(candidate[index].portfolioID) && candidate[index].asset.key == sourceKey {
            ...
            affectedStarts[portfolioID] = min(affectedStarts[portfolioID] ?? candidate[index].timestamp,
    ...
    func importTransactions(_ values: [PortfolioTransaction]) async throws {
        ...
        state.transactions = candidate
        for value in values { touch(value.portfolioID); invalidate(value.portfolioID, from: value.timestamp) }
        try await save()
    }
    ...
    private func touch(_ id: UUID) { if let i = state.portfolios.firstIndex(where: { $0.id == id }) { state.portfolios[i].updatedAt = now() } }
