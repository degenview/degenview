...
actor ScriptStore {
    ...
    private func loadIfNeeded() throws {
        ...
        for dir in dirs where !dir.lastPathComponent.hasPrefix(".") {
            if let script = try decodeIfPresent(LocalScript.self, at: dir.appendingPathComponent("script.json")) {
                metadata[script.id] = script
            }
        }
        loaded = true
    }
    ...
    private func disambiguatedName(_ base: String) -> String {
        let names = Set(metadata.values.map { $0.name.lowercased() })
