...
struct AlertRuntimePersistence: Sendable {
    ...
    func saveSnapshot(_ snapshot: AlertPersistenceSnapshot) {
        ...
        var coordinationError: NSError?
        NSFileCoordinator().coordinate(writingItemAt: snapshotURL, options: .forReplacing, error: &coordinationError) { url in
            try? data.write(to: url, options: .atomic)
        }
    }

    func enqueue(_ command: AlertRuntimeCommand) throws {
        let url = commandsURL.appendingPathComponent("\(command.createdAt.timeIntervalSince1970)-\(command.id.uuidString).json")
    ...
    func pendingCommands() -> [(URL, AlertRuntimeCommand)] {
        let urls = (try? FileManager.default.contentsOfDirectory(at: commandsURL, includingPropertiesForKeys: nil)) ?? []
    ...
    func acknowledge(_ url: URL) {
        var error: NSError?
        NSFileCoordinator().coordinate(writingItemAt: url, options: .forDeleting, error: &error) { try? FileManager.default.removeItem(at: $0) }
    }
    ...
    private func quarantineMalformedFile(_ url: URL) {
        ...
        try? FileManager.default.copyItem(at: url, to: directory.appendingPathComponent("price_alerts.malformed-\(suffix).json"))
    }

    private static let encoder: JSONEncoder = { let value = JSONEncoder(); value.outputFormatting = [.prettyPrinted, .sortedKeys]; return value }()
