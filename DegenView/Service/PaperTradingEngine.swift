...
final class PaperTradingExecutionService: TradingExecutionService, @unchecked Sendable {
    ...
    func modify(_ orderID: PaperOrderID, changes: PaperOrderChanges) async throws { try await engine.modify(orderID, changes: changes) }
}
...
actor PaperTradingEngine {
    ...
    init(snapshot: PaperTradingSnapshot = .empty, quoteMaximumAge: TimeInterval = 30,
    ...
    @discardableResult
    func createAccount(name: String, currency: PaperCurrency = .USD, initialBalance: Decimal = 100_000,
                       settings: PaperAccountSettings = .init()) async throws -> UUID {
        guard initialBalance > 0 else { throw PaperTradingError.invalidQuantity("Initial balance must be greater than zero.") }
        let account = PaperAccount(name: name.isEmpty ? "Paper Trading" : name, baseCurrency: currency,
    ...
    func resetAccount(_ id: UUID, currency: PaperCurrency, initialBalance: Decimal,
                      settings: PaperAccountSettings) async throws {
        guard let index = accountIndex(id) else { throw PaperTradingError.accountNotFound }
        guard initialBalance > 0 else { throw PaperTradingError.invalidQuantity("Initial balance must be greater than zero.") }
        let name = state.accounts[index].name
        state.accounts[index] = PaperAccount(id: id, name: name, baseCurrency: currency,
                                             initialBalance: initialBalance, settings: settings, createdAt: now())
    ...
    func deleteAccount(_ id: UUID) async throws {
        ...
        state.orders.removeAll { $0.accountID == id }; state.fills.removeAll { $0.accountID == id }
        state.positions.removeAll { $0.accountID == id }; state.orderEvents.removeAll { $0.accountID == id }
        state.closedTrades.removeAll { $0.accountID == id }; state.journal.removeAll { $0.accountID == id }
    ...
    func submit(_ request: PaperOrderRequest) async throws -> PaperOrderID {
        ...
        event(order, .placed, "\(request.side.rawValue.uppercased()) \(request.quantity) \(request.instrument.symbol) \(request.type.rawValue) order submitted")
        ...
        do {
            ...
            order.status = .working; order.updatedAt = timestamp
            replace(order)
            ...
            order.status = .rejected; order.rejectionReason = error.localizedDescription; order.updatedAt = timestamp
            replace(order); event(order, .rejected, error.localizedDescription)
            try await save()
            throw error
        }
    }
    ...
    func cancel(_ orderID: UUID) async throws {
        ...
        order.status = .pendingCancel; order.updatedAt = now(); replace(order)
        order.status = .canceled; order.reservedMargin = 0; replace(order)
    ...
    func modify(_ orderID: UUID, changes: PaperOrderChanges) async throws {
        ...
        if let price = changes.limitPrice { try validatePrice(price, instrument: order.instrument); order.limitPrice = price }
        if let price = changes.stopPrice { try validatePrice(price, instrument: order.instrument); order.stopPrice = price }
        order.updatedAt = now()
        order.reservedMargin = try requiredMargin(for: order, account: try account(order.accountID))
        replace(order); event(order, .modified, "PAPER order modified")
    ...
    func metrics(accountID: UUID) -> PaperAccountMetrics {
        guard let account = state.accounts.first(where: { $0.id == accountID }) else {
            return .init(balance: 0, equity: 0, realizedPnL: 0, unrealizedPnL: 0,
        ...
        return .init(balance: account.cashBalance, equity: equity, realizedPnL: account.realizedPnL,
    ...
    func unrealizedPnL(_ position: PaperPosition) -> Decimal {
        ...
        let difference = position.signedQuantity >= 0
    ...
    private func evaluate(orderID: UUID, bracket: (Decimal?, Decimal?)?) throws {
        ...
        case .stopLimit:
            if !order.stopTriggered, stopEligible(order, executable: executable) {
                order.stopTriggered = true; order.updatedAt = now(); replace(order)
    ...
    private func fill(_ order: inout PaperOrder, quantity: Decimal, price: Decimal,
                      source: PaperFillPriceSource, bracket: (Decimal?, Decimal?)?) throws {
        guard quantity > 0, order.status.isWorking else { return }
        let account = try account(order.accountID)
        let commission = commission(for: account.settings.commission, quantity: quantity,
                                    price: price, instrument: order.instrument)
        let fill = PaperFill(id: UUID(), orderID: order.id, accountID: order.accountID,
        ...
        order.reservedMargin = 0; order.updatedAt = now()
        replace(order); state.fills.append(fill)
        ...
        event(order, order.status == .filled ? .filled : .partiallyFilled,
        ...
        if order.status == .filled {
            ...
            if order.role == .entry, let bracket { createBracketChildren(parent: order, takeProfit: bracket.0, stopLoss: bracket.1) }
        }
    }
    ...
    private func applyPosition(_ fill: PaperFill) -> Decimal {
        ...
        if let index = state.positions.firstIndex(where: { $0.accountID == fill.accountID && $0.instrument.key == fill.instrument.key }) {
            ...
            if sameDirection {
                ...
                position.averageEntryPrice = (position.averageEntryPrice * abs(oldSigned) + fill.price * abs(delta)) / total
                position.signedQuantity += delta
                position.commissions += fill.commission; position.updatedAt = fill.timestamp
                state.positions[index] = position
                return 0
            }
            let closedQuantity = min(abs(oldSigned), abs(delta))
            let gross = (oldSigned > 0 ? fill.price - position.averageEntryPrice : position.averageEntryPrice - fill.price)
                * closedQuantity * position.instrument.pointValue
            let allocatedEntryCommission = position.quantity == 0 ? 0 : position.commissions * closedQuantity / position.quantity
            ...
            state.closedTrades.append(.init(id: UUID(), accountID: fill.accountID, instrument: fill.instrument,
                side: oldSigned > 0 ? .long : .short, entryTimestamp: position.openedAt, exitTimestamp: fill.timestamp,
            ...
            position.realizedGrossPnL += gross; position.commissions -= allocatedEntryCommission
            position.signedQuantity += delta; position.updatedAt = fill.timestamp
            if position.signedQuantity == 0 { state.positions.remove(at: index) }
            else if (oldSigned > 0) != (position.signedQuantity > 0) {
                position.averageEntryPrice = fill.price; position.openedAt = fill.timestamp
                position.commissions = openingCommission
                state.positions[index] = position
            } else { state.positions[index] = position }
            resizeProtectiveOrders(accountID: fill.accountID, instrumentKey: fill.instrument.key,
                                   remaining: abs(position.signedQuantity))
            return gross
        }
        state.positions.append(.init(accountID: fill.accountID, instrument: fill.instrument,
    ...
    private func createBracketChildren(parent: PaperOrder, takeProfit: Decimal?, stopLoss: Decimal?) {
        ...
        let group = UUID(), exitSide: PaperOrderSide = parent.side == .buy ? .sell : .buy
        func append(type: PaperOrderType, price: Decimal, role: PaperOrderRole) {
            var child = PaperOrder(id: UUID(), accountID: parent.accountID, instrument: parent.instrument,
            ...
            state.orders.append(child); event(child, .accepted, "PAPER \(role.rawValue) protection activated")
        }
    ...
    private func cancelOCOCounterpart(of order: PaperOrder) {
        ...
        for index in state.orders.indices where state.orders[index].ocoGroupID == group && state.orders[index].id != order.id && state.orders[index].status.isWorking {
            state.orders[index].status = .canceled; state.orders[index].reservedMargin = 0; state.orders[index].updatedAt = now()
    ...
    private func resizeProtectiveOrders(accountID: UUID, instrumentKey: String, remaining: Decimal) {
        for index in state.orders.indices where state.orders[index].accountID == accountID && state.orders[index].instrument.key == instrumentKey && state.orders[index].role != .entry && state.orders[index].status.isWorking {
            if remaining == 0 { state.orders[index].status = .canceled }
            else { state.orders[index].originalQuantity = state.orders[index].filledQuantity + remaining }
    ...
    private func validate(_ request: PaperOrderRequest, account: PaperAccount) throws {
        ...
        if request.instrument.quoteCurrency != account.baseCurrency,
           !([PaperCurrency.USDT, .USDC].contains(request.instrument.quoteCurrency) && account.baseCurrency == .USD) {
            throw PaperTradingError.unsupportedCurrencyConversion(request.instrument.quoteCurrency, account.baseCurrency)
        }
        switch request.type {
        case .market: break
        case .limit:
            guard let price = request.limitPrice else { throw PaperTradingError.invalidPrice("A limit price is required.") }
            try validatePrice(price, instrument: request.instrument)
        case .stop:
            guard let price = request.stopPrice else { throw PaperTradingError.invalidPrice("A stop price is required.") }
            try validatePrice(price, instrument: request.instrument)
        case .stopLimit:
            guard let limit = request.limitPrice, let stop = request.stopPrice else { throw PaperTradingError.invalidPrice("Stop and limit prices are required.") }
            try validatePrice(limit, instrument: request.instrument); try validatePrice(stop, instrument: request.instrument)
        }
    ...
    private func validateStep(_ value: Decimal, increment: Decimal, field: String) throws {
        ...
        guard Decimal.rounded(quotient, scale: 8) == Decimal.rounded(quotient, scale: 0) else {
            throw field == "Price" ? PaperTradingError.invalidPrice("Price must align to tick size \(increment).")
    ...
    private func requiredMargin(for order: PaperOrder, account: PaperAccount) throws -> Decimal {
        ...
        let price: Decimal
        if order.type == .limit || order.type == .stopLimit, let limit = order.limitPrice { price = limit }
        else if order.type == .stop, let stop = order.stopPrice { price = stop }
        else if let quote = state.quotes[order.instrument.key]?.executablePrice(for: order.side)?.0 { price = quote }
        else { throw PaperTradingError.noMarketData }
        let signedPosition = state.positions.first {
    ...
    private func commission(for config: PaperCommissionConfiguration, quantity: Decimal, price: Decimal,
                            instrument: PaperInstrument) -> Decimal {
        switch config {
        case .none: 0
    ...
    private func account(_ id: UUID) throws -> PaperAccount {
        guard let account = state.accounts.first(where: { $0.id == id }) else { throw PaperTradingError.accountNotFound }
        return account
    }
    private func accountIndex(_ id: UUID) -> Int? { state.accounts.firstIndex { $0.id == id } }
    private func replace(_ order: PaperOrder) { if let index = state.orders.firstIndex(where: { $0.id == order.id }) { state.orders[index] = order } }
    private func event(_ order: PaperOrder, _ kind: PaperOrderEventKind, _ message: String) {
        state.orderEvents.append(.init(id: UUID(), orderID: order.id, accountID: order.accountID,
